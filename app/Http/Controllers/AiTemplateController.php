...
class AiTemplateController extends Controller
{
    ...
    public function AiGenerate(Request $request)
    {
        if ($request->ajax()) {
            ...
            $settings = Utility::getChatGPTSetting();
...
$complete = $open_ai->chat([
    'model' => isset($settings['chatgpt_model']) ? $settings['chatgpt_model'] : 'gpt-3.5-turbo',
    'messages' => [
...
if (isset($response['choices'])) {
    foreach ($response['choices'] as $index => $choice) {
        $text .= ($index + 1) . '. ' . ltrim($choice['message']['content']) . "\r\n\r\n";
    }
    $data = trim($text);
    return $data;
} else {
    $data['status'] = 'Error';
    $data['message'] = $response['error']['message'] ?? 'Unknown error';
    return $data;
}
...
if (isset($response['choices'])) {
    if (count($response['choices']) > 1) {
        foreach ($response['choices'] as $choice) {
            $text .= $counter . '. ' . ltrim($choice['message']['content']) . "\r\n\r\n\r\n";
            $counter++;
        }
    } else {
