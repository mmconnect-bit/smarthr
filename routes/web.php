...
// sitemap generation for landing page only
Route::get('/generate-sitemap', function () {
    $sitemap = Sitemap::create();

    // Add landing page specific routes
    $landingPageRoutes = [
        '/',
        '/home',
        '/career',
    ...
    foreach ($landingPageRoutes as $route) {
        $sitemap->add(Url::create(url($route)));
    }

    // Add any custom landing page routes dynamically if they exist
    foreach (\Route::getRoutes()->get() as $route) {
        $uri = $route->uri();
        // Only include public landing page routes (exclude admin, api, dashboard, etc.)
        if (
            !str_contains($uri, '{') && // exclude routes with parameters
            !str_starts_with($uri, 'admin') &&
            !str_starts_with($uri, 'api') &&
            !str_starts_with($uri, 'dashboard') &&
            !str_starts_with($uri, 'login') &&
            !str_starts_with($uri, 'register') &&
            in_array($uri, ['', 'career', 'terms-and-conditions', 'cookie-consent'])) {
    ...
    return 'Landing page sitemap generated!';
});
